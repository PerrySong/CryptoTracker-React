...
export const profile = (formProps, callback) => async dispatch => {
  ...
  axios.post('http://18.204.49.151/user/profile', formProps,
...
export const setting = (formProps, callback) => async dispatch => {
  try{
    axios.put('http://18.204.49.151/user/setting', formProps,
    { headers: { "authorization": `${localStorage.getItem('jwttoken')}` } })
    .then((response) => {
      console.log(response.json);
      callback();
...
export const makeTransaction = (formProps, callback) => async dispatch => {
  try {
    const response = await axios.post('http://18.204.49.151/user/make-transaction', formProps,
    { headers: { "authorization": `${localStorage.getItem('jwttoken')}` } });
